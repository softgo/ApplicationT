...
public class JedisSimpleSessionFactory implements RedisSessionFactory {
	...
	public MulitiRedisSession getMulitiSession() throws RedisException {
		...
		try {
			session = (MulitiRedisSession) Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(),
					new Class[] { MulitiRedisSession.class },
					new JedisMultiSessionProxy(new JedisMulitiRedisSession()));
		}
		catch (Exception e) {
			logger.error("{}", e);
		}
