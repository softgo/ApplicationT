package com.application.base.mongo.factory;
...
public class MongoFactory extends AbstractFactoryBean<Mongo> {
	...
	private Mongo initMongo() throws Exception {
		...
		if (serverList.size() == 0) mongo = new Mongo();
		else if (serverList.size() == 1) {
			if (this.mongoOptions != null) mongo = new Mongo((ServerAddress) serverList.get(0), this.mongoOptions);
			else {
				mongo = new Mongo((ServerAddress) serverList.get(0));
			}
		}
		else if (this.mongoOptions != null) mongo = new Mongo(serverList, this.mongoOptions);
		else {
